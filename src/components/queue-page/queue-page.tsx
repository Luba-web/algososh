...
export const QueuePage: React.FC = () => {
  ...
  const getDequeue = async () => {
    ...
    if (
      queue.current.getTail() === queue.current.getHead() &&
      queue.current.getTail() === 6
    ) {
    ...
    <SolutionLayout title="Очередь">
      <div className={styles.content}>
        ...
        <Button
          ...
          disabled={valueInput === '' || queue.current.getTail() === 6}
          isLoader={disabled.push}
          onClick={getEnqueue}
        />
        <Button
          text={'Удалить'}
